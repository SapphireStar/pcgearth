...
// GridSelectionManager.cpp - 纯Debug版本实现
AGridSelectionManager::AGridSelectionManager()
{
    ...
    GridPlaneMesh->SetCollisionResponseToAllChannels(ECR_Ignore);
    GridPlaneMesh->SetCollisionResponseToChannel(ECC_Visibility, ECR_Block);
}
...
bool AGridSelectionManager::TrySelectGridPoint(const FVector& WorldPosition)
{
    ...
    if (WouldCreateCrossingEdge(GridPoint))
    {
        ...
        DrawDebugString(GetWorld(), GridPoint + FVector(0, 0, 50), TEXT("Crossing Edge!"), nullptr, FColor::Orange, 1.0f);
...
void AGridSelectionManager::DrawDebugGrid()
{
    ...
    DrawDebugBox(GetWorld(), (MinCorner + MaxCorner) * 0.5f, FVector(GridWidth * GridSize * 0.5f, GridHeight * GridSize * 0.5f, 10.0f), 
...
bool AGridSelectionManager::WouldCreateCrossingEdge(const FVector& NewPoint) const
{
    ...
    for (int32 i = 0; i < SelectedGridPoints.Num() - 1; i++)
    {
        ...
        {
            // 绘制交叉警告
            FVector IntersectionPoint = GetLineIntersection(LastPoint, NewPoint, SelectedGridPoints[i], SelectedGridPoints[i + 1]);
            DrawDebugSphere(GetWorld(), IntersectionPoint + FVector(0, 0, 30), 20.0f, 12, FColor::Red, false, 2.0f);
            DrawDebugString(GetWorld(), IntersectionPoint + FVector(0, 0, 60), TEXT("CROSSING!"), nullptr, FColor::Red, 2.0f);
            return true;
        }
    }
